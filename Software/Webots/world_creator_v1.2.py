...
2) Codes modified for admin-permissions in Windows, in Linux they should
   be arranged.
2) Controller paths should be arranged according to Linux system and
   webots configuration.

Updates
1) All controllers updated for mqtt.
2) Arena top camera contoller added.
3) Robot controllers updated to take position and orientation from 
   camera feed.
4) Robot controllers now send translation and rotation infos beside the
   sensor values.
5) Collaboration of game manager and world creator is conducted by mqtt
   manager.
6) Robot tag creation not included but sample code for webots world file
   given below;
   
    Solid {
      translation 0 0.0305 0
        ...
        Shape {
          appearance PBRAppearance {
            baseColorMap ImageTexture {
              url [
                "C:/Users/Korcan/Desktop/marker.png"
              ]
            }
            roughness 1
            metalness 0
   ...
   Only marker image location and name should be corrected and this part
   will be in children of the robot. Also, if marker put in appropriate
   location in Webots files, no need to location url only name of the
   marker image is sufficient.
'''
...
def robot_controller(r_no,aruco_marker_no,supervisor,main,loop):
	'''
	r_no is robot no. main and loop are part of controller scripts which
	necessary for devices and sensors. Robot no(r_no) also can be string
	to give names.
	'''
	...
	robot_controller_main.append('file_to_open = Path("C:/Users/Korcan/Desktop/ME462/t_r_of_robots.txt")\n') #EDIT THISSS
	...
	if supervisor == True:
		...
		robot_controller_main.append('\n')
		robot_controller_main.append('robot_marker = {}\n'.format(aruco_marker_no))
		...
		robot_controller_loop.append('    data_as_string = f.read()\n')
		robot_controller_loop.append('    f.close()\n')
		robot_controller_loop.append('    data = json.loads(data_as_string)\n')
		robot_controller_loop.append('    for d in data:\n')
		robot_controller_loop.append('        if robot_marker in d:\n')
		robot_controller_loop.append('            translation = [d[1]/1000, 0.03, d[2]/1000]\n')
		robot_controller_loop.append('            rotation = [0, 1, 0, -d[3]]\n')
		robot_controller_loop.append('            trans_field.setSFVec3f(translation)\n')
		robot_controller_loop.append('            rot_field.setSFRotation(rotation)\n')
		robot_controller_loop.append('\n')
	
	loop.append('    sen_vals = []\n')
	loop.append('    for e in ds:\n')
	loop.append('        sen_vals.append(e.getValue())\n')
	loop.append('    robot_infos = []\n')
	loop.append('    robot_infos.extend((sen_vals,val_translation,val_rotation))\n')
	loop.append('    robot_infos = json.dumps(robot_infos)\n')
	loop.append('    mqtt.single("W2M_{}",robot_infos ,hostname="192.168.1.7")\n'.format(r_no))#Hostname should be broker adress
...
def robot_creator(x,z,r_no,aruco_marker_no,supervisor,world_text):
	'''
	...
	Top Camera. Robot no(r_no) also can be string to give names.
	'''
	main = []
	loop = []
	...
	#controller of robot
	robot_controller(r_no,aruco_marker_no,supervisor,main,loop)
...
def world_creator(floor_matrix,grid_length,basic_time_step):
	...
	robot_creator(-grid_length,grid_length,"EWA",95,True,contents)
	robot_creator(-grid_length,3*grid_length,"ege",25,True,contents)
